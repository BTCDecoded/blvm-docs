...
```mermaid
flowchart LR
  subgraph paths["Start here"]
    OP[Run a node]
    DEV[Build modules]
    RES[Spec & consensus]
  end

  OP --> OG[Operator guide]
  OG --> INS[Installation]
  INS --> QS[Quick Start]
  QS --> FN[First Node Setup]

  DEV --> DG[Developer guide]
  DG --> FM[First module]
  FM --> MD[Module development]

  RES --> OPaper[Orange Paper]
  OPaper --> FV[Formal verification]
  FV --> CON[Contributing]
```
